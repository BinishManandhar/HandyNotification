...
function toMainSection(authorizationId){
    ...
    if(authorizationId!=undefined){
        ...
        db.transaction(function(tx){
            ...
            tx.executeSql('CREATE TABLE IF NOT EXISTS authorizationValues(id INTEGER,keys,topic,channelID,notificationTitle,notificationMessage,soundName,style,picture,summaryText,priority,vibration1,vibration2,vibration3,vibration4,ledA,ledR,ledG,ledB)');
...
function fillValues(){
    db.transaction(function(tx){
                ...
                function success(tx,rs){
                    ...
                    console.log("Priority: "+rs.rows.item(0).priority);
...
function sendRequest(){
    ...
    if(auth != "0"){
        callPlugin(auth,topic,title,message,androidChannelID,soundName,style,picture,summaryText,priority,vibrationPattern,ledColor);
    }

}

function callPlugin(finalKey,topic,title,message,androidChannelID,soundName,style,picture,summaryText,priority,vibrationPattern,ledColor){
    console.log("Auth: "+finalKey);
    insertValues(finalKey,topic,title,message,androidChannelID,soundName,style,picture,summaryText,priority,vibrationPattern,ledColor);
    Cordova.exec(handySuccess,handyError,'HandyNotificationAndroid', 'pushNotification', [finalKey,topic,title,message,androidChannelID,soundName,style,picture,summaryText,priority,vibrationPattern,ledColor]);
}

function insertValues(finalKey,topic,title,message,androidChannelID,soundName,style,picture,summaryText,priority,vibrationPattern,ledColor){
    var authorizationId;
    db.transaction(function(tx){
        tx.executeSql('SELECT * FROM authorizationKey WHERE keys="'+finalKey+'"',[],
            function success(tx,rs){
                ...
                tx.executeSql('REPLACE INTO authorizationValues VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',[authorizationId,finalKey,topic,androidChannelID,title,message,soundName,style,picture,summaryText,priority,vibrationPattern[0],vibrationPattern[1],vibrationPattern[2],vibrationPattern[3],ledColor[0],ledColor[1],ledColor[2],ledColor[3]]);
